...
WOOCOMMERCE_USER = "ck_3379001f3068ddd004146116bff75be27ea9e88a"
WOOCOMMERCE_PASS = "cs_3b113f2cc3e0740b45d502207a26f6739c80a0b4"

# Gemini API configuration
GEMINI_API_KEY = "AIzaSyDH3IDdDUKtQ0_IvUEHcmaV1_LRlgU6vbU"
...
def generate_seo_content(product_name, product_description, brand_name="", keywords=""):
    """
    Generate SEO content using the Gemini API based on provided product data.
...
"""
    try:
        ...
        print_colored("🚀 Sending request to Gemini API...", Fore.BLUE)
        ...
        if response.status_code != 200:
            print_colored(f"❌ API Error: Status code {response.status_code}", Fore.RED, True)
        ...
        if "candidates" not in response_data or not response_data["candidates"]:
            print_colored("❌ Unexpected API response structure", Fore.RED, True)
            failed_products.append({
                "product_name": product_name,
                "error": "Unexpected API response structure",
                "details": str(response_data)[:200] + "..."
            })
            return None
        
        generated_text = response_data["candidates"][0]["content"]["parts"][0]["text"]
        print_colored("✅ Received content from Gemini API", Fore.GREEN)
...
def update_product_info(product_id, seo_content):
    ...
    Uses the SEO content generated by the Gemini API to update the product.
    """
    try:
...
def main():
    ...
    print_colored(" WooCommerce Product SEO Optimizer ", Fore.CYAN, True)
    print_colored("=" * 80, Fore.CYAN, True)
